...
class LimoPendulumNoNoiseEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        print("[DEBUG]:env0 action",actions[0].item())
    ...
    def _get_observations(self) -> dict:
        ...
        observations = {"policy": obs}
        print("[DEBUG]:env0 obs",self.joint_pos[0, self._pole_dof_idx[0]].item(), self.joint_vel[0, self._pole_dof_idx[0]].item(), self.limo.data.root_link_pos_w[0, 0].item(), self.limo.data.root_com_lin_vel_w[0, 0].item())
        return observations
